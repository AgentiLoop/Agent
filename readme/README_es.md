...
🌐 **Sitio web oficial: [agentiloop.ai](https://agentiloop.ai/es/)** — descargas, documentación, reseñas y noticias.
...
## 🆕 El nuevo del barrio: AgentiLoop CLI ⚡️

**El bucle agéntico, desatado en tu terminal. Mac. Windows. Linux. Tú eliges.**
...
*Creado con AgentiLoop Agent! para Mac. Sí, el agente escribió a sus propios hermanitos.* 🤖✨ ¡Prueba la versión anticipada hoy!
...
Agent! es una app 100 % nativa en Swift 6.2 / SwiftUI que conecta **23 proveedores de LLM** — Claude, Codex, OpenAI, Gemini, Grok, Mistral, Mistral Vibe, DeepSeek, Hugging Face, Z.ai, BigModel, Alibaba DashScope, Qwen, Qwen Code, Meta Muse, MiniMax, OpenRouter, Requesty, A2Agent, OrcaRouter, Ollama (nube y local), vLLM y LM Studio — más **Apple Intelligence** en el dispositivo — a un bucle de tareas autónomo que realmente *hace cosas*: lee tu código, corrige el bug, compila el proyecto de Xcode, hace commit del diff, controla cualquier app de Mac mediante la API de Accesibilidad, ejecuta comandos de shell como tú o como root, te envía los resultados por iMessage y responde a un *«Agent!»* hablado.
...
> ✅ **Nunca necesitas compilar desde el código fuente.** Cada release **y** cada pre-release incluye un binario de Mac precompilado (`.dmg` + `.zip`), construido por CI y **firmado, notarizado y grapado (stapled) por Apple** con el Team ID de AgentiLoop. Como los binarios oficiales llevan un Developer ID real, **el Launch Agent y el Launch Daemon siempre se registran y nunca se «pierden»** — eso solo ocurre con builds desde el código fuente firmados ad-hoc (ver Opción B más abajo). Si tus helpers desaparecieron, instala el [binario del último release](https://github.com/AgentiLoop/Agent/releases/latest).

## Inicio rápido (Compilar desde el código fuente)

> Solo necesario si quieres modificar Agent! en sí. Todos los demás: usad el binario firmado de arriba.
...
|:---:|---|:---:|---|
| <a href="https://fluxionai.world/register?source=github&campaign=aiagent&promo=AIAGENT"><img src="../docs/sponsors/fluxion-ai-logo.png" width="64" height="64" alt="Fluxion AI logo" /></a> | <a href="https://fluxionai.world/register?source=github&campaign=aiagent&promo=AIAGENT"><b>Fluxion&nbsp;AI</b></a> | <img src="../docs/badges/sponsor-silver-offset.svg" width="66" height="29" alt="Silver" /> | Acceso fiable y económico a GPT, Claude y otros modelos de IA líderes a través de una API unificada. Ahorra hasta un 70 % frente a los precios oficiales de la API y recibe $3 en créditos de API al registrarte a través de [este enlace](https://fluxionai.world/register?source=github&campaign=aiagent&promo=AIAGENT) (código promocional `AIAGENT`). |
...
| **Meta Muse** | | De pago | Muse Spark mediante la Meta Model API (`api.meta.ai`), clave `LLM_…` compatible con OpenAI |
