...
🌐 **Site officiel : [agentiloop.ai](https://agentiloop.ai/fr/)** — téléchargements, documentation, avis et actualités.
...
Agent! est une app 100 % native Swift 6.2 / SwiftUI qui relie **23 fournisseurs de LLM** — Claude, Codex, OpenAI, Gemini, Grok, Mistral, Mistral Vibe, DeepSeek, Hugging Face, Z.ai, BigModel, Alibaba DashScope, Qwen, Qwen Code, Meta Muse, MiniMax, OpenRouter, Requesty, A2Agent, OrcaRouter, Ollama (cloud et local), vLLM et LM Studio — plus **Apple Intelligence** sur l'appareil — à une boucle de tâches autonome qui *agit vraiment* : elle lit votre code, corrige le bug, compile le projet Xcode, committe le diff, pilote n'importe quelle app Mac via l'API d'Accessibilité, exécute des commandes shell en votre nom ou en root, vous envoie les résultats par iMessage et répond à un *« Agent! »* prononcé à voix haute.
...
| Logo | Sponsor | &nbsp;&nbsp;&nbsp;Niveau&nbsp;&nbsp;&nbsp; | |
|:---:|---|:---:|---|
| <a href="https://fluxionai.world/register?source=github&campaign=aiagent&promo=AIAGENT"><img src="../docs/sponsors/fluxion-ai-logo.png" width="64" height="64" alt="Fluxion AI logo" /></a> | <a href="https://fluxionai.world/register?source=github&campaign=aiagent&promo=AIAGENT"><b>Fluxion&nbsp;AI</b></a> | <img src="../docs/badges/sponsor-silver-offset.svg" width="66" height="29" alt="Silver" /> | Un accès fiable et économique à GPT, Claude et d'autres modèles d'IA de pointe via une API unifiée. Économisez jusqu'à 70 % par rapport aux tarifs officiels des API et recevez 3 $ de crédits API en vous inscrivant via [ce lien](https://fluxionai.world/register?source=github&campaign=aiagent&promo=AIAGENT) (code promo `AIAGENT`). |
...
| Fournisseur | &nbsp;&nbsp;&nbsp;Sponsor&nbsp;&nbsp;&nbsp; | Coût | Idéal pour |
|---|:---:|---|---|
...
| **Meta Muse** | | Payant | Muse Spark via la Meta Model API (`api.meta.ai`), clé `LLM_…` compatible OpenAI |
| **MiniMax** | | Bon marché | Contexte de 1M tokens |
| **Mistral** / **Mistral Vibe** | | Payant | Cloud open-weight, code, produit agent |
...
| **Alibaba DashScope** | | Bon marché | Qwen 3.8 via DashScope |
| **Qwen** (qwen.ai Token Plan) | | Abonnement | QwenCloud Token Plan (clé `sk-sp-`) : qwen3.8-max, qwen3.7-plus, GLM-5.2, DeepSeek V4 |
| **Qwen Code** | | Abonnement | Alibaba Coding Plan (clé `sk-sp-`) : qwen3-coder-plus, qwen3.7-plus, GLM-5, Kimi K2.5, MiniMax-M2.5 |
...
**Conventions réelles de `AGENT_SCRIPT_ARGS` dans les scripts fournis** (`~/Documents/AgentScript/agents/Sources/Scripts/`) :

| Script | `arguments:` passé par le LLM | Style |
|---|---|---|
...
| `QuitApps` | `excluded=Xcode,Agent,Terminal` | `key=value` avec une liste |
| `NowPlaying` | `json=true,artwork=true` | `key=value,…` |
| `ArchiveXcode` | `/path/to/Project.xcodeproj MyScheme 469UCUB275` | positionnel, séparé par des espaces (scheme/teamID détectés automatiquement s'ils sont omis) |
| `CreateDmg` | `--app /path/to/App.app --output /path/out.dmg --name "My App" --compress` | style drapeaux, séparé par des espaces, guillemets respectés |

**Entrée / sortie JSON — un mécanisme SÉPARÉ des variables d'environnement.** Les variables d'environnement sont exportées par Agent! dans le processus ; les fichiers JSON sont de simples fichiers sur disque que le *script lui-même* lit et écrit avec `FileManager` / `JSONSerialization`. Agent! ne les crée pas, ne les transmet pas et ne les analyse pas. Deux schémas réels tirés des scripts fournis :

*1. Entrée JSON uniquement (`SendMessage`)* — aucun argument d'environnement ; le script exige `SendMessage_input.json` et renvoie `1` s'il est absent :

```json
// ~/Documents/AgentScript/json/SendMessage_input.json   (écrit par le LLM via file(action:"write") avant l'exécution)
{ "recipient": "Maman", "message": "À la maison à 18 h", "imagePath": "~/Pictures/Photos Library.photoslibrary/originals/A/IMG_0001.jpeg" }

// ~/Documents/AgentScript/json/SendMessage_output.json  (écrit par le script)
{ "success": true, "timestamp": "2026-09-03T21:14:02Z", "recipient": "Maman", "message": "À la maison à 18 h" }
// en cas d'échec : { "success": false, "timestamp": "…", "error": "Missing required field: recipient" }
```

```swift
// SendMessage.swift — comment le script le lit
...
*2. Arguments d'environnement pour les options, JSON pour la sortie structurée (`TodayEvents`, `NowPlaying`, `CheckMail`, `ListReminders`)* — les options viennent de `AGENT_SCRIPT_ARGS` (ou du fichier optionnel `<Name>_input.json`) ; avec `json=true` le script écrit `<Name>_output.json` en plus de la sortie lisible sur stdout qui revient au LLM :
...
```swift
// TodayEvents.swift — comment le script l'écrit
...
>
> « AgentiLoop Agent! » et le logo AgentiLoop Agent! sont des marques déposées de AgentiLoop.ai, une société de Logos InkPen LLC. L'utilisation de ces marques nécessite une autorisation écrite préalable. La licence PolyForm Noncommercial ci-dessous accorde des droits uniquement sur le code source — elle **n'accorde aucun** droit de marque.
>
...
>
> Les binaires compilés, installateurs, builds signés, et artefacts de release distribués via les GitHub Releases de ce projet, [AgentiLoop.ai](https://AgentiLoop.ai), ou tout autre canal officiel, sont l'œuvre protégée par le droit d'auteur de AgentiLoop.ai, une socieÌteÌ de Logos InkPen LLC et **ne sont pas** couverts par la licence PolyForm Noncommercial qui régit le code source. Tous les droits sur les binaires officiels — y compris le nom « AgentiLoop Agent! », le logo, l'identité de signature de code, et le Developer ID — sont réservés.
>
> Copyright © 2026 AgentiLoop.ai, une société de Logos InkPen LLC. Tous droits réservés.
>
> Vous êtes libre de compiler vos propres binaires à partir des sources pour un usage non commercial sous la licence PolyForm Noncommercial, à condition de ne pas utiliser le nom « AgentiLoop Agent! », le logo, ou la marque pour identifier votre produit.
>
